...
export default function EmployeeWorkingHoursSettings() {
  const api = useApi();
  const { user } = useAuth();
    ...
    const fetchWorkingHours = async () => {
      ...
      try {
        const { data } = await api.get<{ workingHours: WorkingHours | null }>(
          `/employee/${user.id}/working-hours`
        );
  ...
  const handleSave = useCallback(async () => {
    ...
    setMessage(null);
    try {
      await api.put(`/employee/${user.id}/working-hours`, { workingHours });
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-white/60">
          Configurează zilele și orele în care ești disponibil pentru rezervări.
        </p>
      </div>
            ...
            >
              ...
              {daySchedule.enabled && (
                <div className="space-y-3">
                  {/* Primul slot - perioada principală de lucru */}
